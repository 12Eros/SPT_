...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    seq_file_name = seq.name.replace('/', '_')
    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            if isinstance(data[0], (dict, OrderedDict)):
                ...
                for obj_id, d in data_dict.items():
                    bbox_file = os.path.join(base_results_path, f"{seq_file_name}_001.txt")
                    save_bb(bbox_file, d)
            else:
                # Single-object mode
                bbox_file = os.path.join(base_results_path, f"{seq_file_name}_001.txt")
        ...
        if key == 'all_scores':
            ...
            if isinstance(data[0], dict):
                ...
                for obj_id, d in data_dict.items():
                    timings_file = os.path.join(base_results_path, f"{seq_file_name}_001_time.value")
            ...
            if isinstance(data[0], dict):
                data_dict = _convert_dict(data)
                for obj_id, d in data_dict.items():
                    confidence_file = os.path.join(base_results_path, f"{seq_file_name}_001_confidence.value")
